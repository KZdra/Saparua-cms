...
if (isset($_POST['judul'], $_POST['artikel'], $_POST['vid'])) {
    $judul = $_POST['judul'];
    $artikel = $_POST['artikel'];
    $vid = $_POST['vid'];

    // Use prepared statements to prevent SQL injection
    $proses = $pdo->prepare("INSERT INTO docvid (judul, artikel, vid) VALUES (?, ?, ?)");

    try {
        // Bind parameters and execute the query
        $proses->bindParam(1, $judul);
        $proses->bindParam(2, $artikel);
        $proses->bindParam(3, $vid);
        $proses->execute();
...
<div class="container p-8 mx-auto">
    <form  method="post" enctype="multipart/form-data" class="max-w-md p-8 mx-auto bg-white border rounded-md shadow-md">
        ...
        <!-- Article Photo -->
        <div class="mb-4">
        <label for="vid" class="block mb-2 text-sm font-medium text-gray-600">VIDEO EMBED LINK</label>
            <input type="text" name="vid" id="vid" class="w-full px-4 py-2 border rounded-md focus:outline-none focus:border-blue-500" required>
        </div>
